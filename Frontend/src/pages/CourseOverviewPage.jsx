...
const CourseOverviewPage = () => {
  ...
  useEffect(() => {
    const getCourse = async () => {
      try {
        ...
        // DEBUG
        console.log("Full YouTube Response:", data);
        console.log("Items:", data.items);
        console.log("API KEY:", import.meta.env.VITE_YT_V3_API_KEY);
  ...
  console.log("First Lecture:", course.curriculum?.[0]?.lectures?.[0]);
  console.log("VideoId:", course.curriculum?.[0]?.lectures?.[0]?.contentDetails?.videoId);

  const firstLectureUrl = course.curriculum?.[0]?.lectures?.[0]?.contentDetails?.videoId
    ? APP_ROUTES.courseContent.replace(':courseId', course.id).replace(':lectureId', course.curriculum[0].lectures[0].contentDetails.videoId)
    : '#';

  console.log("First Lecture URL:", firstLectureUrl);
